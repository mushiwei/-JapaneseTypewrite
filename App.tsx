...
const App: React.FC = () => {
  ...
  const startTraining = (level: string) => {
    let items: JLPTItem[] = [];
    if (level === 'KANA') {
      // 五十音图模式：从全量库中随机抽取 20 个单字，实现高频反馈练习
      items = [...KANA_DATA].sort(() => 0.5 - Math.random()).slice(0, 20);
    } else {
    ...
    <div className="max-w-6xl mx-auto py-16 px-6 space-y-20">
      ...
      <section className="manga-border p-12 bg-black text-white relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
          <h2 className="text-4xl font-black uppercase font-zcool">为什么选择严格训练？</h2>
          <div className="grid md:grid-cols-3 gap-12 text-sm font-bold opacity-80 leading-relaxed">
            <div className="space-y-2">
              <div className="text-pink-500 text-lg">01. 瞬时反应</div>
              <p>JLPT考试时间紧张，你必须对单词达到“生理反射”级别的熟悉度。本系统通过打字速度（WPM）强迫你跳过中文翻译过程。</p>
            </div>
            <div className="space-y-2">
              <div className="text-pink-500 text-lg">02. 听音辨位</div>
              <p>原生 Gemini TTS 模拟真实语境发音，让你在录入的同时建立正确的重音（Accent）感。</p>
            </div>
            <div className="space-y-2">
              <div className="text-pink-500 text-lg">03. 零容错率</div>
              <p>严苛的报错机制会记录每一次击键。通过不断提高准确率，你将掌握日文录入的节奏感。</p>
            </div>
          </div>
        </div>
      </section>
    ...
    <div className="min-h-screen bg-stone-100">
      ...
      {mode === AppMode.TRAINING && (
        <div className="py-12 px-4">
          <TypingView items={practiceItems} title={`${currentLevel} 专项能力突破`} onFinish={handleFinish} />
        </div>
      )}
